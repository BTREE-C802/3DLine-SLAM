...
namespace L3DPP
{
        ...
        superpixels_(superpixels)
    {
        ...
        RtKinv_  = Rt_*Kinv_;
        C_ = Rt_ * (-1.0 * t_);
	KRt_.block(0,0,3,3) = R_;
	KRt_.block(0,3,3,1) = t_;
	KRt_ = K_*KRt_;
        k_ = 0.0f;
        median_depth_ = 0.0f;
        median_sigma_ = 0.0f;
        ...
        for(size_t r=0; r<3; ++r)median_depth_
    ...
    //-----------------------------------------------------------------------------
    void View::UpdateView(const unsigned int id,
	...
	KRt_ = K_*KRt_;
        C_ = Rt_ * (-1.0 * t_);
	if( initial_median_depth_ == 0.0f)
	{
		initial_median_depth_ = fmax(fabs(median_depth),L3D_EPS);//更新中值深度
	}
#ifdef L3DPP_CUDA
        C_f3_ = make_float3(C_.x(),C_.y(),C_.z());
        // RtKinv -> data array
